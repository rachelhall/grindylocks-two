...
export const accountRouter = createTRPCRouter({
    getAccountByUsername: publicProcedure.input(z.object({ username: z.string() })).query(async ({ ctx, input }) => {
        const account = await ctx.prisma.account.findUnique({ where: { username: input.username }, include: { posts: true, follow_requests: true, followedBy: true, following: true, sponsors: true } })
